...
- **Live `research_brief` (phase 1+2):** off-golden `depth=quick|standard` runs **search → fetch/extract N pages (capped) → synthesize → quote gate** via an injectable `ResearchProvider`. Local synthesizer (`local-quoted-v1`) emits `{ claim, quote, source_url }[]` with verbatim **query-tied** sentences from page text (not arbitrary homepage leads). Tests use `MockProvider` (no network). Production with `LIVE_RESEARCH=1` uses `LocalHttpProvider` (DDG HTML URLs + HTTP extract). **Deep** stays sample. Pipeline failure → sample. `PARALLEL_API_KEY` / `EXA_API_KEY` are placeholders and **unused**.
...
- **Honest page-body cache (P4):** extracted bodies keyed by URL + content hash + `as_of` + volatility TTL (`PAGE_CACHE_TTL_MS`). Always emit `source: cached|live` on live-pipeline sources / body meta. Bypass on `as_of_hint`, `FORCE_LIVE=1`, or `PAGE_CACHE_BYPASS=1`. Charge gate unchanged.
- **MCP Tasks (P3b stub):** sync path remains default. Dual-path Tasks gated off (`ENABLE_MCP_TASKS`); host support **unconfirmed** — see `docs/mcp-tasks.md`. Do not fake CreateTaskResult.
